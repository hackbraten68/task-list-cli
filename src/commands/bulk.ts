...
export async function bulkMarkCommand(status?: TaskStatus, ids?: string) {
  if (!ids) {
    UI.error(
      "Please provide task IDs. Usage: lazytask bulk-mark <status> <ids>",
    );
...
export async function bulkUpdateCommand(ids?: string, options?: {
  ...
  if (!ids) {
    UI.error(
      "Please provide task IDs. Usage: lazytask bulk-update <ids> [options]",
    );
    UI.info(
      'Example: lazytask bulk-update "1,2,3" --priority high --tags "urgent"',
    );
    return;
  }
  ...
  if (options?.tags) {
    changes.tags = options.tags.split(",").map((t) => t.trim()).filter((t) =>
      t
    );
  }
