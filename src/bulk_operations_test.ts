...
import { Task, TaskStatus, TaskPriority } from "../src/types.ts";
import { bulkMarkTasks, bulkDeleteTasks, bulkUpdateTasks } from "../src/storage.ts";
import { parseTaskIds, validateTaskIds, prepareBulkOperation, getTaskSummaries } from "../src/utils/task-selection.ts";
...
Deno.test("Bulk Storage Operations Tests", async (t) => {
    ...
    await t.step("BulkResult interface structure", () => {
        ...
        assert(true, "BulkResult interface tests would require file system mocking");
    });
});
...
Deno.test("Integration Tests", async (t) => {
    await t.step("ID range parsing integration", () => {
        ...
        assertEquals(result.errors.length, 0, "Valid existing IDs should not produce errors");
        assertEquals(result.taskIds, [1, 2, 3], "Should return valid task IDs");

        const invalidResult = prepareBulkOperation("1,999", mockTasks);
        assert(invalidResult.errors.length > 0, "Invalid task IDs should produce errors");
