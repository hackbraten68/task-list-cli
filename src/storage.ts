import { Task, BulkResult, ExportOptions, ImportOptions, ImportResult } from "./types.ts";
...
export async function exportTasks(options: ExportOptions): Promise<void> {
    ...
    const outputPath = options.outputPath || `lazytask-export-${new Date().toISOString().split('T')[0]}.${options.format}`;
    ...
    if (options.format === 'json') {
        ...
        // CSV format
        const headers = ['id', 'description', 'details', 'status', 'priority', 'dueDate', 'tags', 'createdAt', 'updatedAt'];
 ...
 */
export async function importTasks(options: ImportOptions): Promise<ImportResult> {
    try {
        // Read input file
        ...
        if (options.format === 'json') {
            ...
            if (!Array.isArray(importedTasks)) {
                return { success: false, message: 'JSON file must contain an array of tasks' };
            }
        } else {
            // CSV parsing
            const lines = content.trim().split('\n');
            if (lines.length < 2) {
                return { success: false, message: 'CSV file must have at least a header row and one data row' };
            }

            const headers = lines[0].split(',').map(h => h.trim());
            const expectedHeaders = ['id', 'description', 'details', 'status', 'priority', 'dueDate', 'tags', 'createdAt', 'updatedAt'];
        ...
        if (options.validateOnly) {
            return {
                success: true,
                message: `Validation successful. ${validTasks.length} tasks would be imported.`,
        ...
        if (options.mode === 'replace') {
            ...
            return {
                success: true,
                message: `Successfully replaced all tasks with ${validTasks.length} imported tasks.`,
                importedCount: validTasks.length
            };
        } else {
            // Merge mode
 ...
 */
function validateTask(rawTask: any, lineNumber: number): { valid: boolean; task?: Task; error?: string } {
    const errors: string[] = [];
    ...
    if (!rawTask.status || !['todo', 'in-progress', 'done'].includes(rawTask.status)) {
        errors.push('status is required and must be one of: todo, in-progress, done');
    }

    if (!rawTask.priority || !['low', 'medium', 'high', 'critical'].includes(rawTask.priority)) {
    ...
    if (rawTask.tags && typeof rawTask.tags === 'string') {
        // Split CSV tags
        rawTask.tags = rawTask.tags.split(';').map((t: string) => t.trim()).filter((t: string) => t.length > 0);
    }
