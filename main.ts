...
if (import.meta.main) {
  await new Command()
    ...
    .option(
      "--fuzzy",
      "Use fuzzy matching for search (approximate string matching)",
    )
    .option(
      "--sort-by <field:string>",
      "Sort by field (due-date, priority, status, created, updated, description)",
    )
    .option("--sort-order <order:string>", "Sort order (asc or desc)", {
      default: "asc",
    })
    ...
    .option("-f, --format <format:string>", "Export format (json or csv)", {
      default: "json",
    })
    ...
    .option("-f, --format <format:string>", "Import format (json or csv)", {
      default: "json",
    })
    .option("-m, --mode <mode:string>", "Import mode (merge or replace)", {
      default: "merge",
    })
